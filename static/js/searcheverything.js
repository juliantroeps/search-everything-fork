var SearchEverything = ( function ( $ ) {
    var r = {
            ...
            resizeSearchInput: function () {
                ...
                input.css( {
                    width:
                        input.closest( "div" ).outerWidth( true ) -
                        input.next( "a" ).outerWidth( true )
                } );
            },
            ...
            displayOwnResults: function ( holder, data ) {
                ...
                $.each( data, function ( i, result ) {
                    var listItem = $(
                        '<li><div title="Click to insert link into post."><h6></h6><a href="" target="_blank"></a><p></p></div></li>'
                    );
                    if ( i > 4 ) {
                        return;
                    }
                    ...
                    listItem
                        .find( "a" )
                        .text( r.urlDomain( result.guid ) )
                        .prop( "title", result.title || "Title missing" )
                        .prop( "href", result.guid );
            ...
            performSearch: function () {
                var input = $( "#se-metabox-text" ),
                    results = $(
                        '<div id="se-metabox-results"><div id="se-metabox-own-results" class="se-metabox-results-list"><h4>Results from your blog</h4><p class="se-instructions">Click to insert link into post.</p><ul></ul></div><div class="se-spinner"></div></div>'
                    ),
                    count = 0;
                ...
                $.ajax( {
                    ...
                    success: function ( data ) {
                        ...
                        if ( data.own.length === 0 ) {
                            $( "#se-metabox-results" ).append(
                                '<p class="se-no-results">It seems we haven\'t found any results for search term <strong>' +
                                input.prop( "value" ) +
                                " on your blog</strong>.</p>"
                            );
                        } else {
                            ownResults.show();
                            ...
                            ownResults
                                .find( "h4" )
                                .text( ownResults.find( "h4" ).text() + " (" + count + ")" );
                        }
                    },
                    error: function ( xhr ) {
                        $( ".se-spinner, .se-no-results" ).remove();
                        $( "#se-metabox-results" ).append(
                            '<p class="se-no-results">There was something wrong with the search. Please try again. If this happens a lot, please check out our <a href="http://wordpress.org/support/plugin/search-everything" target="_blank">Support forum</a>.</p>'
                        );
                    }
                } );
                ...
                results.on( "click", ".se-settings-link", function () {
                    $( this )
                        .parent()
                        .text( "Thanks. Please refresh this page." );
                } );
            },
            urlDomain: function ( url ) {
                // http://stackoverflow.com/a/8498668
            ...
            initResultBehaviour: function () {
                ...
                metabox.on( "click", "#se-metabox-own-results li", function ( ev ) {
                    var insertHtml = $( html ),
                        ...
                        inserted = $(
                            'a.se-box[href="' + listItem.data( "guid" ) + '"]',
                            ( tinyMCE && tinyMCE.activeEditor.contentWindow.document ) ||
                            document
                        );
                    ...
                    if ( inserted.length ) {
                        ...
                        insertHtml
                            .find( ".se-box-heading-title" )
                            .text(
                                listItem.data( "post_title" ) || "Title missing".substring( 0, 50 )
                            );
                        insertHtml
                            .find( ".se-box-heading-domain" )
                            .text( "(" + r.urlDomain( listItem.data( "guid" ) ) + ")" );
                        insertHtml
                            .find( ".se-box-text" )
                            .text( r.extractText( listItem, "post_content" ) );
                ...
                metabox.on( "click", ".se-metabox-results-list h4", function () {
                    $( this )
                        .closest( ".se-metabox-results-list" )
                        .toggleClass( "se-hidden" );
                } );
            }
        },
        u = {
